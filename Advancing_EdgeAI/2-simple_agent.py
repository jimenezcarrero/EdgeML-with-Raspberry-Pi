...
def ask_ollama_for_classification(user_input):
    """
    Ask Ollama to classify whether the query is a multiplication request or a general question.
    """
    classification_prompt = f"""
    Analyze the following query and determine if it's asking for multiplication or if it's a general question.
...
def process_query(user_input):
    ...
    then either performing multiplication or sending it back as a general question.
    """
    ...
    if classification.get("type") == "multiplication":
        ...
        if len(numbers) >= 2:
            ...
            return "I understood you wanted multiplication, but couldn't extract the numbers properly."
    else:
